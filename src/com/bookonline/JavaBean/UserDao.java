...
public class UserDao 
{
	...
	public int insert(Connection connection, UserBean user)
		...
		String sql="insert into user_tab(userName,password,repassword,gender,tel,email,admin) values(?,?,?,?,?,?,?)";
                try 
                {
    ...
    public boolean check(Connection connection, String userName, String password)
    {
		...
		String sql = "select userName,uid,tel from user_tab where userName = ? and password = ?";
		try
		{
			...
			if(resultSet.next())
			{
				...
				usertel=resultSet.getString("tel");
				return value;
			}
 ...
 // 将查询出的值放入UserBean对象中 并返回该对象
    public UserBean getUser(Connection connection, String userName)
    {
          ...
          String sql="select userName,password,gender,tel,email,admin from user_tab where userName=?";
          PreparedStatement statement=null;
     ...
     // Update 获得用户修改后的信息 进行数据库的修改操作
        public int Update(Connection connection, UserBean user)
		{
			int value = -1;
			PreparedStatement statement = null;
			String sql = "update user_tab set gender = ?, tel = ?, email = ? where userName = ?";
        ...
        public int UpdatePass(Connection connection,String password,String userName)
	{
		int value = -1;
		PreparedStatement statement = null;
		String sql = "update user_tab set password = ? where userName = ?";
